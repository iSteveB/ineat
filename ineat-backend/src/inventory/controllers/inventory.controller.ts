...
import { AddManualProductDto, ProductCreatedResponseDto } from '../dto/add-manual-product.dto';
import { JwtAuthGuard } from '../../auth/guards/jwt-auth.guard';
import { Request } from 'express';

// Interface pour l'utilisateur authentifié (à adapter selon votre implémentation d'auth)
...
export class InventoryController {
  ...
  @ApiOperation({ 
    ...
    description: 'Permet à un utilisateur d\'ajouter un produit à son inventaire en saisissant manuellement les informations'
  })
  @ApiBody({ 
  ...
  @ApiResponse({ 
    ...
    schema: {
      example: {
        statusCode: 400,
        message: ['Le nom du produit est obligatoire', 'La quantité doit être supérieure à 0'],
        error: 'Bad Request'
      }
    }
  })
  ...
  @ApiResponse({ 
    status: 409, 
    description: 'Produit déjà présent dans l\'inventaire avec les mêmes caractéristiques',
    schema: {
      example: {
        statusCode: 409,
        message: 'Ce produit existe déjà dans votre inventaire avec les mêmes caractéristiques',
        error: 'Conflict'
      }
    }
  })
  async addManualProduct(
    ...
    @Body(new ValidationPipe({ transform: true, whitelist: true })) addProductDto: AddManualProductDto,
  ): Promise<ProductCreatedResponseDto> {
    return await this.inventoryService.addManualProduct(req.user.id, addProductDto);
  }
  ...
  @ApiOperation({ 
    ...
    description: 'Récupère tous les produits de l\'inventaire de l\'utilisateur avec possibilité de filtrer'
  })
  @ApiQuery({ 
    name: 'category', 
    required: false, 
        ...
        properties: {
          ...
          product: {
            ...
            properties: {
              ...
              nutriscore: { type: 'string', enum: ['A', 'B', 'C', 'D', 'E'], nullable: true },
              ecoScore: { type: 'string', enum: ['A', 'B', 'C', 'D', 'E'], nullable: true },
              category: {
                type: 'object',
                properties: {
                  id: { type: 'string', format: 'uuid' },
  ...
  @ApiOperation({ 
    ...
    description: 'Permet de modifier les informations d\'un produit dans l\'inventaire'
  })
  @ApiParam({ 
    name: 'id', 
  ...
  @ApiOperation({ 
    ...
    description: 'Supprime définitivement un produit de l\'inventaire de l\'utilisateur'
  })
  @ApiParam({ 
    name: 'id', 
  ...
  async removeInventoryItem(
    ...
    return await this.inventoryService.removeInventoryItem(req.user.id, inventoryItemId);
  }
  ...
  @ApiOperation({ 
    ...
    description: 'Récupère des statistiques sur l\'inventaire de l\'utilisateur (nombre de produits, répartition par catégorie, etc.)'
  })
  @ApiResponse({ 
    status: 200, 
    ...
    schema: {
      type: 'object',
      properties: {
        totalItems: { type: 'number', description: 'Nombre total d\'éléments dans l\'inventaire' },
        totalValue: { type: 'number', description: 'Valeur totale de l\'inventaire (si prix renseignés)' },
        expiringInWeek: { type: 'number', description: 'Nombre de produits périment dans la semaine' },
        categoriesBreakdown: {
          type: 'array',
          items: {
            type: 'object',
