...
export const OFF_ERROR_MESSAGES = {
  NETWORK_ERROR: 'Impossible de contacter OpenFoodFacts. Vérifiez votre connexion internet.',
  API_ERROR: 'Erreur du serveur OpenFoodFacts. Veuillez réessayer plus tard.',
  PRODUCT_NOT_FOUND: 'Produit non trouvé dans la base de données OpenFoodFacts.',
...
export interface OffError {
  type: 'NETWORK_ERROR' | 'API_ERROR' | 'PRODUCT_NOT_FOUND' | 'INVALID_BARCODE' | 'RATE_LIMIT' | 'UNKNOWN_ERROR';
  message: string;
  details?: unknown;
}
...
export type OffImageSize = typeof OFF_IMAGE_SIZES[keyof typeof OFF_IMAGE_SIZES];
...
export const OFF_UTILS = {
   ...
   */
  buildUserAgent: (appName: string, version: string, contactEmail: string): string => {
