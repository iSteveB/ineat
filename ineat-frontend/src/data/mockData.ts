...
export const mockProductsWithExpiryStatus: ProductWithExpiryStatus[] =
	mockProducts.map((product) => ({
		...product,
		expiryStatus: calculateExpiryStatus(product.expiryDate as Date),
	}));
...
export const calculateAverageNutriscore = (products: Product[]): number => {
	...
	const total = products.reduce(
		(sum, product) => {
			if (product.nutriscore && product.nutriscore in nutriScoreValues) {
				return sum + nutriScoreValues[product.nutriscore as keyof typeof nutriScoreValues];
			}
			return sum;
		},
		0
