import { EcoScore, NutriScore, UnitType } from '@/schemas/productSchema';
...
// export { AddManualProductPage } from '@/components/inventory/AddManualProductPage';
...
export const getCategoryLabel = (categorySlug: string): string => {
  const category = INVENTORY_CATEGORIES.find(cat => cat.value === categorySlug);
  return category?.label || categorySlug;
};

export const getStorageLocationLabel = (location: string): string => {
  const storageLocation = INVENTORY_STORAGE_LOCATIONS.find(loc => loc.value === location);
...
export const getExpiryStatus = (expiryDate?: string): 'fresh' | 'warning' | 'danger' | 'expired' => {
