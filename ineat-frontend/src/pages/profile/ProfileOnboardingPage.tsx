...
const GOALS: Array<{ value: PrimaryGoal; label: string; description: string }> =
  [
    {
      value: "REDUCE_WASTE",
      label: "Réduire le gaspillage",
      description: "Consommer les produits qui arrivent bientôt à expiration.",
    },
    {
      value: "SAVE_MONEY",
      label: "Économiser",
      description: "Mieux suivre le budget et les dépenses alimentaires.",
    },
    {
      value: "EAT_BETTER",
      label: "Mieux manger",
      description: "Privilégier des propositions adaptées à mes préférences.",
    },
    {
      value: "FIND_MEAL_IDEAS",
      label: "Trouver des idées",
      description: "Transformer plus facilement mon inventaire en repas.",
    },
  ];
...
export default function ProfileOnboardingPage() {
  ...
  const finish = async (saveAnswers: boolean) => {
    ...
    try {
      ...
      toast.success(
        saveAnswers
          ? "Votre profil est personnalisé."
          : "Vous pourrez personnaliser votre profil plus tard.",
      );
    ...
    <div className="mx-auto flex min-h-[calc(100vh-8rem)] max-w-4xl items-center px-4 py-8">
      <Card className="w-full">
        <CardHeader>
          ...
          <CardDescription>
            Trois réponses suffisent et restent modifiables dans vos paramètres.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form className="space-y-8" onSubmit={handleSubmit}>
            <section className="space-y-3">
              <Label htmlFor="onboarding-servings">
                Pour combien de personnes cuisinez-vous habituellement ?
              </Label>
              <Input
                id="onboarding-servings"
                type="number"
                min={1}
                max={20}
            ...
            <section className="space-y-3">
              <div>
                ...
                <p className="text-sm text-neutral-500">
                  Sélectionnez « Aucune » pour vider la liste.
                </p>
              </div>
              ...
              <div className="flex gap-2 rounded-md bg-warning-50/20 p-3 text-sm text-neutral-700">
                ...
                <span>
                  Les suggestions InEat ne remplacent jamais la vérification des
                  ingrédients et des étiquettes.
                </span>
              </div>
            </section>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                {GOALS.map((goal) => (
                  <button
                    key={goal.value}
                    type="button"
                    aria-pressed={primaryGoal === goal.value}
                    onClick={() => setPrimaryGoal(goal.value)}
                    className={`rounded-lg border p-4 text-left transition ${primaryGoal === goal.value ? "border-success-600 bg-success-50/10" : "border-neutral-200 hover:border-success-300"}`}
                  >
            ...
            <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
              ...
              <Button
                type="submit"
                disabled={
                  isSaving || defaultServings < 1 || defaultServings > 20
                }
              >
