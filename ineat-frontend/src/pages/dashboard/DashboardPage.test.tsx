...
describe("DashboardPage", () => {
  ...
  it.each([
    ...
    "personnalise le dashboard pour l’objectif %s",
    async (primaryGoal, title, href, firstWidget) => {
      (useAuthStore as unknown as ReturnType<typeof vi.fn>).mockReturnValue({
        user: { firstName: "Camille", primaryGoal },
      });
      ...
      expect(screen.getByText("Agir maintenant")).toHaveAttribute("href", href);
      const widget = screen.getByTestId(firstWidget);
      expect(widget.parentElement?.firstElementChild).toBe(widget);
    },
  );
  ...
  it("affiche les widgets avec inventaire, budget, récents et produits à expirer", async () => {
    ...
    expect(screen.getByTestId("recent-products-widget")).toHaveTextContent("3");
  ...
  it("affiche un état d’erreur lorsque le dashboard ne peut pas charger", async () => {
    (
      inventoryService.getInventory as ReturnType<typeof vi.fn>
    ).mockRejectedValue(new Error("API indisponible"));
    (
      inventoryService.getInventoryStats as ReturnType<typeof vi.fn>
    ).mockResolvedValue({
      totalItems: 0,
